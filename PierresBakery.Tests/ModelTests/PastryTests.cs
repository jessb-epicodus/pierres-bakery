...
namespace PierresBakery.Tests {
  ...
  public class PastryTests {
    ...
    NameOfMethodWeAreTesting_DescriptionOfBehavior_ExpectedReturnValue() {  //  MSTest tests are actually methods.  They are public, which allows the tools running our tests to access them. We add void because they don't explicitly return anything.
      ...
      Pastry testPastry = new Pastry();  //    ie. create an instance of class OR dont if using static method
      ...
      Assert.Method(expectedrestult, testPastry.Method(testvalue)); //   write our first assertion using a method.  The first argument is what we expect the result of the test to be. The second is the expression to be evaluated.
    }
  }
}